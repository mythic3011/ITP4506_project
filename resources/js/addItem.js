...
function getNewVehicleId() {
  ...
  document.getElementById("NewVehicle-id").textContent = "FK000SE1";
  // Simulating an API call to get a new Vehicle ID
  fetch("/api/getNewVehicleId")
    .then((response) => response.json())
    .then((data) => {
      document.getElementById("NewVehicle-id").textContent = data.id;
    })
...
function addItem() {
  ...
  formData.append("id", document.getElementById("NewVehicle-id").textContent);
